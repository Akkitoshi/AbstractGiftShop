...
namespace AbstractGiftShopServiceImplementDataBase.Implementations
{
    public class ImplemeneterServiceDB : IReportService
    {
        ...
        public void SaveGiftPrice(ReportBindingModel model)
        {
            ...
            try
            {
               ...
               missing);
                ...
                var table = document.Tables.Add(rangeTable, Gifts.Count, 2, ref
               missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = Gifts[i].GiftName;
                    table.Cell(i + 1, 2).Range.Text = Gifts[i].Price.ToString();
                }
        ...
        public List<StocksLoadViewModel> GetStocksLoad()
        {
           ...
           .ToList(),
            stock => stock,
           stockMaterials => stockMaterials.SStock,
           (stock, stockCompList) =>
            new StocksLoadViewModel
            {
                StockName = stock.SStockName,
                TotalCount = stockCompList.Sum(r => r.Count),
                Materialss = stockCompList.Select(r => new Tuple<string,
    int>(r.Materials.MaterialsName, r.Count))
            })
            .ToList();
        }
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        if (elem.Materialss.Count() > 0)
                        {
                            ...
                            var excelBorder =
                             ...
                             excelcells.get_Offset(elem.Materialss.Count()
                            - 1, 1));
 ...
 excel.Workbooks[1].Save();
        ...
        public List<SClientOrdersModel> GetSClientOrders(ReportBindingModel model)
        {
        ...
        public void SaveClientOrders(ReportBindingModel model)
        {
           ...
           iTextSharp.text.Font.BOLD);
            ...
            table.AddCell(new PdfPCell(new Phrase("Подарок", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            //заполняем таблицу
            var list = GetSClientOrders(model);
